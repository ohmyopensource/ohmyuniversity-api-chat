 ...
 * Custom queries use Spring Data derived query methods to keep the code readable without explicit
 * JPQL.
 */
@Repository
public interface ChatChannelRepository extends JpaRepository<ChatChannel, UUID> {
   ...
   *
   * @param externalChannelId the external channel ID (e.g. {@code analisi-i-unimol-2026-1})
  ...
  /**
   * Finds all channels with the given status whose {@code closesAt} is before the specified
   * instant.
   *
   ...
   * @param threshold the reference instant; channels with {@code closesAt} before this are
   *                  considered expired
